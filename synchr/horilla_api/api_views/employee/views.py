...
class EmployeeTypeAPIView(APIView):
    ...
    def post(self, request):
        if permission_check(request, "employee.add_employeetype") is False:
    ...
    def put(self, request, pk):
        if permission_check(request, "employee.change_employeetype") is False:
            return Response({"error": "No permission"}, status=401)
    ...
    def delete(self, request, pk):
        if permission_check(request, "employee.delete_employeetype") is False:
            return Response({"error": "No permission"}, status=401)
        employee_type = object_check(EmployeeType, pk)
        if employee_type is None:
            return Response({"error": "EmployeeType not found"}, status=404)
...
class EmployeeTagAPIView(APIView):
    ...
    def get(self, request, pk=None):
        if pk:
            obj = object_check(EmployeeTag, pk)
            if not obj: return Response(status=404)
            return Response(EmployeeTagSerializer(obj).data)
        objs = EmployeeTag.objects.all()
        return Response(EmployeeTagSerializer(objs, many=True).data)
    def post(self, request):
        serializer = EmployeeTagSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=201)
        return Response(serializer.errors, status=400)
    def put(self, request, pk):
        obj = object_check(EmployeeTag, pk)
        if not obj: return Response(status=404)
        serializer = EmployeeTagSerializer(obj, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=400)
