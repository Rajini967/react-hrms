...
class WorkTypeRequest(HorillaModel):
    ...
    # Handle None values to prevent errors
     if not self.requested_date or not self.employee_id:
        return False
        
    # First query - check range overlaps (only if both dates exist)
     if self.requested_till:
        approved_work_type_requests_range = WorkTypeRequest.objects.filter(
            employee_id=self.employee_id,
            approved=True,
        ...
        if self.id:
            approved_work_type_requests_range = approved_work_type_requests_range.exclude(id=self.id)
        if approved_work_type_requests_range.exists():
            return True
    
    # Second query - check if any approved request covers our start date
     approved_work_type_requests = WorkTypeRequest.objects.filter(
        employee_id=self.employee_id,
        approved=True,
        canceled=False,
     ...
     if self.id:
        approved_work_type_requests = approved_work_type_requests.exclude(id=self.id)
     if approved_work_type_requests.exists():
        return True
    
    # Third query - check if any approved request covers our end date (if exists)
     if self.requested_till:
        approved_work_type_requests_2 = WorkTypeRequest.objects.filter(
            employee_id=self.employee_id,
            approved=True,
        ...
        if self.id:
            approved_work_type_requests_2 = approved_work_type_requests_2.exclude(id=self.id)
        if approved_work_type_requests_2.exists():
            return True
    
    # Fourth query - check permanent requests on same date
     approved_permanent_req = WorkTypeRequest.objects.filter(
        employee_id=self.employee_id,
        approved=True,
        canceled=False,
     ...
     if self.id:
        approved_permanent_req = approved_permanent_req.exclude(id=self.id)
     if approved_permanent_req.exists():
        return True
...
class WorkTypeRequestComment(HorillaModel):
    ...
    def __str__(self) -> str:
        if self.employee_id:
          last_name = self.employee_id.employee_last_name if self.employee_id.employee_last_name else ""
          return f"{self.employee_id.employee_first_name} {last_name} - {self.requested_date}"
        else:
          return f"Shift Request - {self.requested_date}"
