...
from employee.models import Actiontype, Employee, ProfileEditFeature
from horilla_audit.models import AccountBlockUnblock, AuditTag
...
from ...api_serializers.base.serializers import (
    ...
    WorkTypeSerializer,
    AnnouncementExpireSerializer,
    DynamicPaginationSerializer,
    DynamicEmailConfigurationSerializer,
    AccountBlockUnblockSerializer,
    ProfileEditFeatureSerializer,
    GroupSerializer,
    PermissionSerializer,
    AuditTagSerializer,
    GoogleDriveBackupSerializer,
    DefaultAccessibilitySerializer,
)
...
class GeneralSettingsView(APIView):
    ...
    def get(self, request):
        announcement = AnnouncementExpire.objects.first()
        pagination = DynamicPagination.objects.filter(user_id=request.user).first()
        block_unblock = AccountBlockUnblock.objects.first()
        profile_edit = ProfileEditFeature.objects.first()

        data = {
            "announcement_expire_days": announcement.days if announcement else 30,
            "pagination": pagination.pagination if pagination else 50,
            "restrict_login": block_unblock.is_enabled if block_unblock else False,
            "restrict_profile_edit": profile_edit.is_enabled if profile_edit else False,
    ...
    def post(self, request):
        ...
        if "announcement_expire_days" in data:
            ...
            instance.days = data["announcement_expire_days"]
            instance.save()
            
        if "pagination" in data:
            instance, _ = DynamicPagination.objects.get_or_create(user_id=request.user)
            instance.pagination = data["pagination"]
            instance.save()
            
        if "restrict_login" in data:
            instance, _ = AccountBlockUnblock.objects.get_or_create()
            instance.is_enabled = data["restrict_login"]
            instance.save()
            
        if "restrict_profile_edit" in data:
            instance, _ = ProfileEditFeature.objects.get_or_create()
            instance.is_enabled = data["restrict_profile_edit"]
            instance.save()
            
        return Response({"status": "success"})


class DynamicEmailConfigurationView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        instance = DynamicEmailConfiguration.objects.filter(company_id=request.user.employee_get.get_company()).first()
        if not instance:
            instance = DynamicEmailConfiguration.objects.filter(is_primary=True).first()
        
        if instance:
            serializer = DynamicEmailConfigurationSerializer(instance)
            return Response(serializer.data)
        return Response({}, status=404)

    def post(self, request):
        company = request.user.employee_get.get_company()
        instance = DynamicEmailConfiguration.objects.filter(company_id=company).first()
        serializer = DynamicEmailConfigurationSerializer(instance, data=request.data, partial=True)
        if not instance:
            serializer = DynamicEmailConfigurationSerializer(data=request.data)
...
class AuditTagView(APIView):
    permission_classes = [IsAuthenticated]
    serializer_class = AuditTagSerializer

    def get(self, request, pk=None):
        if pk:
            obj = object_check(AuditTag, pk)
            if not obj: return Response(status=404)
            return Response(self.serializer_class(obj).data)
        objs = AuditTag.objects.all()
        return Response(self.serializer_class(objs, many=True).data)
...
class GoogleDriveBackupView(APIView):
    permission_classes = [IsAuthenticated]
    serializer_class = GoogleDriveBackupSerializer
...
class DefaultAccessibilityView(APIView):
    permission_classes = [IsAuthenticated]
    serializer_class = DefaultAccessibilitySerializer

    def get(self, request):
        feature = request.GET.get("feature")
        if feature:
            instance = DefaultAccessibility.objects.filter(feature=feature).first()
            if instance:
                return Response(self.serializer_class(instance).data)
            return Response({}, status=404)
        instances = DefaultAccessibility.objects.all()
...
class EmployeePermissionView(APIView):
    ...
    def get(self, request, pk):
        ...
        permissions = employee.employee_user_id.user_permissions.all()
    ...
    def post(self, request, pk):
        ...
        employee.employee_user_id.user_permissions.set(permission_ids)
        return Response({"status": "success"})
