...
const RotatingWorkTypeAssign = ({ employeeId }) => {
    ...
    const fetchAssignments = async () => {
        try {
            const url = employeeId
                ? `/base/rotating-worktype-assigns/?employee_id=${employeeId}`
                : '/base/rotating-worktype-assigns/';
    ...
    const handleOpen = (assignment = null) => {
        if (assignment) {
            ...
            setFormData({
                ...
                rotating_work_type_id: assignment.rotating_work_type_id,
    ...
    const filteredAssignments = assignments.filter(a =>
        ...
        (a.rotating_work_type_name || '').toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const handleSubmit = async () => {
        try {
            if (editingId) {
                await api.put(`/base/rotating-worktype-assigns/${editingId}/`, formData);
                    ...
                    <Table>
                        <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                            <TableRow>
                                ...
                                <TableCell>Rotating Work Type</TableCell>
                        ...
                        <TableBody>
                            {filteredAssignments.map((assignment) => (
                                <TableRow key={assignment.id} hover>
                                    ...
                                    <TableCell>{assignment.rotating_work_type_name}</TableCell>
                            ...
                            >
                                <Box sx={{
                                    ...
                                    <Typography variant="body2" color="primary" fontWeight="600" sx={{ mb: 2 }}>
                                        {assignment.rotating_work_type_name}
                                    </Typography>
            ...
            <Dialog open={openModal} onClose={handleClose} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        ...
                        <Grid item xs={12}>
                            <FormControl fullWidth required>
                                <InputLabel>Rotating Work Type</InputLabel>
                                <Select
                                    name="rotating_work_type_id"
                                    value={formData.rotating_work_type_id}
                                    onChange={handleChange}
                                    label="Rotating Work Type"
                                >
                                    {rotatingWorkTypes.map((type) => (
