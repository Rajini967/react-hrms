...
const WorkTypeRequests = () => {
    ...
    const fetchRequests = async () => {
        try {
            const response = await api.get('/base/worktype-requests/');
    ...
    const fetchWorkTypes = async () => {
        try {
            const response = await api.get('/base/worktypes/');
            setWorkTypes(response.data.results || response.data);
        } catch (error) {
    ...
    return (
        <div style={{ padding: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>Work Type Requests</h1>
                    <p style={{ color: '#718096', margin: '0.25rem 0 0 0' }}>Manage work type change requests from employees.</p>
                </div>
                <button
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', backgroundColor: '#fff', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ position: 'relative' }}>
                    <FaSearch style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: '#a0aec0' }} />
                    <input
                        type="text"
                        placeholder="Search employee..."
                        style={{ padding: '0.6rem 1rem 0.6rem 2.5rem', borderRadius: '8px', border: '1px solid #e2e8f0', width: '250px' }}
                    />
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ backgroundColor: '#f8fafc' }}>
                        <tr>
                            ...
                            <th style={{ textAlign: 'left', padding: '1rem', color: '#4a5568', fontWeight: 600, borderBottom: '1px solid #e2e8f0' }}>Work Type</th>
                            <th style={{ textAlign: 'left', padding: '1rem', color: '#4a5568', fontWeight: 600, borderBottom: '1px solid #e2e8f0' }}>Requested At</th>
                    ...
                    <tbody>
                        {filteredRequests.map(req => (
                            <tr key={req.id} style={{ borderBottom: '1px solid #edf2f7' }}>
                                ...
                                <td style={{ padding: '1rem' }}>
                                    <div style={{ color: '#4a5568' }}>{req.work_type_name}</div>
                                    {req.is_permanent_work_type && <span style={{ fontSize: '0.7rem', backgroundColor: '#ebf8ff', color: '#3182ce', padding: '2px 6px', borderRadius: '4px' }}>Permanent</span>}
                                </td>
                                <td style={{ padding: '1rem', color: '#4a5568' }}>{new Date(req.requested_date).toLocaleDateString()}</td>
                                <td style={{ padding: '1rem', color: '#4a5568' }}>
                                    {req.is_permanent_work_type ? 'N/A' : `${req.requested_date} to ${req.requested_till}`}
                                </td>
                                ...
                                <td style={{ padding: '1rem', textAlign: 'right' }}>
                                    ...
                                    <button style={{ marginLeft: '0.5rem', padding: '0.4rem', border: 'none', background: 'none', color: '#cbd5e0', cursor: 'not-allowed' }}>
                                        <FaHistory />
                                    </button>
                                </td>
                            </tr>
            ...
            {isModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    ...
                    >
                        <h2 style={{ marginBottom: '1.5rem' }}>New Work Type Request</h2>
                                ...
                                {!formData.is_permanent_work_type && (
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <input
                                            type="date"
                                            className="form-control"
                                            required={!formData.is_permanent_work_type}
                            ...
                            <div style={{ marginBottom: '1.5rem' }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                                    <input
                                        ...
                                        onChange={(e) => setFormData({ ...formData, is_permanent_work_type: e.target.checked })}
                                    />
                                    <span>Is Permanent Work Type</span>
                                </label>
                            </div>
                            <div style={{ marginBottom: '1.5rem' }}>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Reason / Description</label>
                                <textarea
                                    className="form-control"
                                    rows="3"
